...
class Vtiger_ExportData_Action extends Vtiger_Mass_Action {
	...
	function ExportData(Vtiger_Request $request) {
		...
		for ($j = 0; $j < $db->num_rows($result); $j++) {
			...
			$sql = "SELECT * FROM vtiger_contactdetails INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_contactdetails.contactid WHERE vtiger_contactdetails.phone = '".$entries[$j]['phone']."' AND vtiger_crmentity.deleted = 0";
			$sql_result = $db->pquery($sql);
			$sql_rowData = $db->query_result_rowdata($sql_result, 0);
			
		    $sql = "UPDATE vtiger_crmentity SET deleted = '1' WHERE crmid = ".$sql_rowData['contactid'];
			$sql_result = $db->pquery($sql);
			}
		}
		...
		if($moduleName == "Contacts"){
		$translatedHeader[0]=$translatedHeaders[2];
		$translatedHeader[1]=$translatedHeaders[3];
		$translatedHeader[2]=$translatedHeaders[4];
		$translatedHeader[3]=$translatedHeaders[13];
		$translatedHeader[4]=$translatedHeaders[29];
		$translatedHeader[5]=$translatedHeaders[30];
		$translatedHeader[6]=$translatedHeaders[31];
		$translatedHeader[7]=$translatedHeaders[32];
		$translatedHeader[8]=$translatedHeaders[35];
	
		$entrie[0]['contact_no']=$entries[0]['contact_no'];
		$entrie[0]['lastname']=$entries[0]['lastname'];
		$entrie[0]['phone']=$entries[0]['phone'];
		$entrie[0]['email']=$entries[0]['email'];
		$entrie[0]['cf_852']=$entries[0]['cf_852'];
		$entrie[0]['cf_854']=$entries[0]['cf_854'];
		$entrie[0]['cf_856']=$entries[0]['cf_856'];
		$entrie[0]['cf_858']=$entries[0]['cf_858'];
		$entrie[0]['cf_866']=$entries[0]['cf_866'];
		
		$this->output($request, $translatedHeader, $entrie);
